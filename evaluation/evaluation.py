...
class Evaluation:
    ...
    def read_embedding_file(self, embedding_file_path):
        ...
        model = gensim.models.KeyedVectors.load_word2vec_format(embedding_file_path, binary=False)

        # Get the number of nodes
        N = len(model.vocab)

        # Generate the embedding dictionary
        node2embedding = {str(node): model[str(node)] for node in range(N)}
